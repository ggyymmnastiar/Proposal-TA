...
Berdasarkan latar belakang yang telah dijelaskan, maka permasalahan yang akan diteliti dapat dirumuskan sebagai berikut:

\begin{enumerate}
    \item Bagaimana menentukan sentimen (positif, negatif, atau netral) dari umpan balik pengguna terhadap produk UMKM menggunakan metode \emph{Natural Language Processing} (NLP)?
    \item Bagaimana mengklasifikasikan aspek atau tema utama dari umpan balik pengguna untuk memberikan wawasan strategis bagi pelaku UMKM?
    \item Sejauh mana akurasi model NLP dalam menganalisis sentimen dan klasifikasi aspek teks berbahasa Indonesia?
\end{enumerate}
...
\begin{enumerate}
    \item Menganalisis sentimen dari umpan balik pengguna terhadap produk UMKM menggunakan pendekatan NLP.
    \item Mengklasifikasikan umpan balik pengguna berdasarkan aspek atau tema yang relevan.
    \item Mengevaluasi kinerja model NLP dalam konteks teks berbahasa Indonesia.
\end{enumerate}
...
Hasil penelitian ini diharapkan dapat memperkaya literatur terkait penerapan \emph{Natural Language Processing} (NLP) untuk analisis sentimen dan klasifikasi aspek teks berbahasa Indonesia, khususnya pada domain UMKM.

\subsection{Manfaat Praktis}
Penelitian ini dapat membantu pelaku UMKM memahami persepsi pelanggan secara lebih sistematis melalui analisis data teks, sehingga dapat dijadikan dasar untuk meningkatkan kualitas produk, pelayanan, dan strategi pemasaran berbasis data.

\section{Batasan Masalah}

Untuk menjaga fokus penelitian, batasan masalah yang diterapkan adalah sebagai berikut:

\begin{enumerate}
    \item Data yang digunakan berupa umpan balik pengguna (review atau komentar) dari platform digital tertentu yang relevan dengan produk UMKM.
    \item Bahasa yang dianalisis terbatas pada bahasa Indonesia.
    \item Metode NLP yang digunakan berfokus pada analisis sentimen dan klasifikasi aspek, tanpa membahas rekomendasi atau prediksi perilaku pengguna.
\end{enumerate}

\section{Metodologi Penelitian}

Penelitian ini menggunakan pendekatan kuantitatif dengan metode analisis teks berbasis \emph{Natural Language Processing} (NLP). Secara umum, proses penelitian terdiri atas beberapa tahapan utama, yaitu pengumpulan data, pra-pemrosesan teks, analisis sentimen, klasifikasi aspek, serta evaluasi hasil model.  

Data penelitian akan diperoleh dari ulasan atau umpan balik pengguna terhadap produk UMKM yang tersedia di platform daring. Data tersebut kemudian akan melalui tahap pra-pemrosesan yang meliputi pembersihan teks, normalisasi, tokenisasi, dan penghapusan kata-kata umum (\emph{stopwords}).  

Tahap berikutnya adalah analisis sentimen menggunakan model pembelajaran mesin (\emph{machine learning}) untuk menentukan polaritas opini pengguna (positif, negatif, atau netral). Selain itu, dilakukan pula klasifikasi aspek untuk mengidentifikasi topik utama dalam setiap umpan balik, seperti kualitas produk, harga, layanan pelanggan, dan pengiriman.  

Model yang digunakan akan dievaluasi menggunakan metrik akurasi, presisi, \emph{recall}, dan \emph{F1-score} guna mengukur kinerja sistem secara objektif. Hasil dari analisis ini diharapkan dapat memberikan wawasan yang berguna bagi pelaku UMKM dalam memahami persepsi konsumen serta mendukung pengambilan keputusan berbasis data.
